...
func (db *DB) GetUserStats() (map[string]int, error) {
        ...
        err = db.QueryRow("SELECT COUNT(*) FROM users WHERE is_active = true AND user_type != 'suspended'").Scan(&count)
        if err != nil {
                return nil, err
        }
        stats["active"] = count
        ...
        // Suspended users
        err = db.QueryRow("SELECT COUNT(*) FROM users WHERE user_type = 'suspended'").Scan(&count)
        if err != nil {
                return nil, err
        }
        ...
        // Verified users
        err = db.QueryRow("SELECT COUNT(*) FROM users WHERE is_verified = true AND is_active = true").Scan(&count)
        if err != nil {
                return nil, err
        }
        ...
        // Admin users
        err = db.QueryRow("SELECT COUNT(*) FROM users WHERE user_type = 'admin' AND is_active = true").Scan(&count)
        if err != nil {
                return nil, err
        }
        stats["admin"] = count
